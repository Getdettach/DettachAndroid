...
public class JJSMS implements Serializable {
	...
	public static final String INITIAL_MESSAGE = "#D/The person you are trying to reach has set status ";

	public static final String INITIAL_MESSAGE1 = "DETACH/The person you are calling to reach is ";
	...
	{
		//System.out.println("jjsmsStr = " + jjsmsStr);
	...
	private void initExtras()
	{
		...
		if (this.hasExtraParams(this.jjsmsStr))
		{
			...
			try {
				...
				Log.e(TAG,"The app failed to extract the data portion of jjsms string.");
		...
		{
			Log.e(TAG,"raw jjsmsStr has extras: else " );
	...
	{
		String extraStr = rawjjSMSstr.substring(rawjjSMSstr.indexOf('?') + 1,rawjjSMSstr.length());
		return extraStr;
	}
	...
	{
		Log.e(TAG, extras.toString());
		Log.e(TAG, "The converted extras are:" + convertExtrasToString());
		return "#D/" + this.getCommand().toString() + "?"
